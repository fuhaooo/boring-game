...
import { RpcProvider, Account, ec, Contract, json, CallData, constants, stark } from "starknet";
...
const STARKNET_PROVIDER_URL = process.env.STARKNET_PROVIDER_URL || "https://starknet-sepolia.infura.io/v3/";
...
async function main() {
  try {
    ...
    console.log(yellow("目标钱包地址:"), "0x011bf31D662FF2638447c5BF401f97b6CaFF5DAbcBEE772dbcEDFC6319b66Bba");
    ...
    const sierraFile = files.find(f => f.includes("BoringGame.contract_class.json"));
    const casmFile = files.find(f => f.includes("BoringGame.compiled_contract_class.json"));
    ...
    // 读取合约文件内容
    const compiledSierra = JSON.parse(fs.readFileSync(path.join(contractsPath, sierraFile), "utf8"));
    const compiledCasm = JSON.parse(fs.readFileSync(path.join(contractsPath, casmFile), "utf8"));
    ...
    console.log(green("龙珠NFT URL:"), "ipfs://bafybeiciojjygr67dngemgpp3us5dvy7gpze3weuw2qqeeh4lfmpxizrru");
    console.log(green("可通过网关访问:"), "https://blush-rainy-constrictor-734.mypinata.cloud/ipfs/bafybeiciojjygr67dngemgpp3us5dvy7gpze3weuw2qqeeh4lfmpxizrru");
