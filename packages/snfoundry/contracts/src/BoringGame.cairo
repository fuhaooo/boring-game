...
pub trait IBoringGame<TContractState> {
    ...
    fn get_player_info(
        self: @TContractState, player: starknet::ContractAddress,
    ) -> (u256, u256, u256);
    fn get_achievement(self: @TContractState, id: u8) -> (u8, felt252, felt252, u256);
    fn is_achievement_unlocked(
        self: @TContractState, player: starknet::ContractAddress, achievement_id: u8,
    ) -> bool;
}
...
pub mod BoringGame {
    ...
    fn constructor(ref self: ContractState, owner: ContractAddress) {
        ...
        self
            ...
            .write(
                1,
                Achievement {
                    id: 1, name: 'Beginner', description: 'Reach 100 points', requirement: 100,
                },
            );
        self
            .achievements
            .write(
                2,
                Achievement {
                    id: 2, name: 'Intermediate', description: 'Reach 500 points', requirement: 500,
                },
            );
        self
            .achievements
            .write(
                3,
                Achievement {
                    id: 3, name: 'Advanced', description: 'Reach 1000 points', requirement: 1000,
                },
            );
        ...
        self
            ...
            .write(
                ...
                Achievement {
                    id: 4,
                    name: 'I Have Seen Dragon',
                    description: 'Collect all seven Dragon Balls',
                    requirement: 0,
                },
            );
    ...
    impl BoringGameImpl of IBoringGame<ContractState> {
        ...
        fn start_game(ref self: ContractState) -> bool {
            ...
            let target_wallet: ContractAddress = TARGET_WALLET_ADDRESS
                .try_into()
                .expect('Invalid address');
            ...
            // 发送事件
            self.emit(GameStarted { player: caller, timestamp: get_block_timestamp() });
        ...
        fn mint_achievement_nft(ref self: ContractState, achievement_id: u8) -> bool {
            ...
            self
                .emit(
                    NFTMinted { player: caller, achievement_id: achievement_id, nft_url: nft_url },
                );
    ...
    impl InternalFunctions of InternalFunctionsTrait {
        ...
        fn _check_achievements(
            ref self: ContractState, player_address: ContractAddress, score: u256,
        ) {
            // 遍历检查积分成就（ID 1-3）
            let mut i: u8 = 1;
            loop {
                ...
                if score >= achievement.requirement {
                    // 发送成就解锁事件
                    self.emit(AchievementUnlocked { player: player_address, achievement_id: i });
                }
                i += 1;
            }
        }
