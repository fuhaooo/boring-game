...
const BubbleWrap: React.FC<BubbleWrapProps> = ({ onLevelComplete, onAllLevelsComplete }) => {
  const { t } = useLanguage();
  const [currentLevel, setCurrentLevel] = useState(1);
  const [bubbles, setBubbles] = useState<{id: number, isPopped: boolean, isUnpopping: boolean}[]>([]);
  ...
  const popBubble = (bubbleId: number) => {
    ...
    if (popSoundRef.current) {
      ...
      popSoundRef.current.play().catch(e => console.log('Audio play failed:', e));
    }
    ...
    setTimeout(() => {
      ...
      if (allPopped) {
        ...
        // 只在非挑战模式下恢复气泡
        // 设置 unpop 定时器
        const unpopDelay = Math.max(100, (bubbles.length * 1000) / currentLevel);
        
        const timer = setTimeout(() => {
          setBubbles(prev => {
            const poppedBubbles = prev.filter(b => 
              b.isPopped && b.id !== bubbleId
            );
            ...
            if (unpopSoundRef.current) {
              ...
              unpopSoundRef.current.play().catch(e => console.log('Audio play failed:', e));
            }
          ...
          // 移除这个定时器
          unpopTimersRef.current = unpopTimersRef.current.filter(t => t !== timer);
        }, unpopDelay);
    ...
    <div className="fixed inset-0 z-30 pointer-events-none">
      ...
      <div className="absolute top-20 left-5 pointer-events-auto">
        <div className="bg-black bg-opacity-90 text-white p-4 rounded-lg max-w-xs border-2 border-white shadow-xl">
          ...
          {!gameStarted && !showLevelComplete && !challengeMode && !challengeCompleted ? (
            <div>
              <p className="text-sm mb-2">
                {t('Level')} {currentLevel}/5 - {getLevelConfig(currentLevel).bubbleCount} {t('bubbles')}
              </p>
            ...
            <div>
              <p className="text-green-400 font-bold mb-2">{t('Level Complete')}!</p>
              {completedLevels.length === 5 && (
            ...
            <div>
              <p className="text-green-400 font-bold mb-2">{t('Challenge Complete')}!</p>
            ...
            <div>
              <p className="text-sm mb-2">
                {t('Challenge Mode')} - {bubbles.filter(b => b.isPopped).length}/{bubbles.length} {t('popped')}
              </p>
            ...
            <div>
              <p className="text-sm mb-2">
                {t('Level')} {currentLevel}/5 - {bubbles.filter(b => b.isPopped).length}/{bubbles.length} {t('popped')}
              </p>
              <button
                onClick={resetLevel}
                className="bg-orange-500 hover:bg-orange-600 text-white px-3 py-1 rounded text-sm"
            ...
            >
              ...
              <span 
                style={{
                  ...
                  width: '35%',
                  height: '25%',
                  background: 'radial-gradient(circle, rgba(255,255,255,0.98) 0%, transparent 80%)',
                  borderRadius: '50%',
                  pointerEvents: 'none',
                ...
                <>
                  ...
                  <span 
                    style={{
                      ...
                      width: '30%',
                      height: '30%',
                      background: 'radial-gradient(ellipse, rgba(139,69,19,0.8) 0%, rgba(160,82,45,0.4) 70%)',
                      borderRadius: '50%',
                    }}
                  />
                  {/* 破裂线条1 */}
                  ...
                  <span 
                    style={{
                      ...
                      width: '60%',
                      height: '60%',
                      background: 'radial-gradient(ellipse, rgba(101,67,33,0.5) 0%, transparent 80%)',
                      borderRadius: '50%',
                    }}
                  />
                </>
      ...
      <style jsx>{`
        ...
        @keyframes pop {
          ...
          15% { transform: scale(1.1); filter: brightness(1.3); }
          40% { transform: scale(0.8); filter: brightness(0.9); }
          60% { transform: scale(0.85); filter: brightness(0.88); }
          100% { transform: scale(0.85); filter: brightness(0.85); }
        }
