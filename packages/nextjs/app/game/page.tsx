...
const APPROVE_SELECTOR = "0x219209e083275171774dab1df80982e9df2096516f06319c5c6d71ae0a8480c";

// 导入NFT预览图片地址
const NFT_IMAGE_URL = "https://blush-rainy-constrictor-734.mypinata.cloud/ipfs/bafybeiciojjygr67dngemgpp3us5dvy7gpze3weuw2qqeeh4lfmpxizrru";
...
const BoringGame = () => {
  ...
  const [hasCompletedDragonBallQuest, setHasCompletedDragonBallQuest] = useState(false); // 是否已完成龙珠任务
  ...
  const checkAllowance = async () => {
    ...
    try {
      ...
      // 获取BoringGame合约地址
      const boringGameAddress = "0x3e4aa5993cc45e05bd5ffa6789d883e1632e4a5df73caec16d8e4010c517719";
      
      // 调用allowance函数检查授权金额
      const result = await erc20Contract.call("allowance", [address, boringGameAddress]);
  ...
  const approveSTRK = async () => {
    ...
    try {
      // 获取BoringGame合约地址
      const spender = "0x3e4aa5993cc45e05bd5ffa6789d883e1632e4a5df73caec16d8e4010c517719";
  ...
  // 记录分数
  const recordScore = useCallback(async (args: { args: [number] }) => {
    try {
      if (!address) {
        console.error("钱包未连接");
        return false;
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-6 rounded-lg shadow-xl w-[600px] max-w-full">
          <h3 className="text-xl font-bold mb-4">铸造NFT: {achievement?.name}</h3>
  ...
  useEffect(() => {
    ...
    if (dragonBalls.length === 7 && !showToothless && !hasCompletedDragonBallQuest) {
  ...
  }, [dragonBalls.length, showToothless, unlockedAchievements, hasCompletedDragonBallQuest]);
            ...
            <ul className="list-disc pl-6 space-y-2">
              ...
              <li>积分达到特定门槛时会解锁购买选项：
                <ul className="list-circle pl-6 mt-1">
                  <li><strong>50分</strong>：移动图标 - 每次碰到浏览器边界增加1积分（最多可购买10个）</li>
                  <li>购买满10个图标后可以花费<strong>2000分</strong>升级，升级后每次碰撞增加5分并有音效</li>
              ...
              <li>可收集七龙珠：
                <ul className="list-circle pl-6 mt-1">
                  ...
                  <li>召唤无牙后可以花费<strong>5000分</strong>升级龙珠，升级后碰撞有音效</li>
                  <li>龙珠升级后，新购买的龙珠价格从<strong>5000分</strong>开始翻倍增长</li>
                </ul>
              </li>
        ...
        <div className="flex flex-col items-center w-full max-w-5xl mx-auto relative z-10">
          ...
          <div className="flex justify-center flex-wrap gap-4 mt-8">
            ...
            <div className={`relative rounded-lg overflow-hidden w-24 h-24 border-2 ${
              ...
              <button 
                onClick={movingIconCount < 10 ? purchaseMovingIcon : upgradeMovingIcon}
                disabled={(movingIconCount < 10 && score < UNLOCK_THRESHOLDS.movingIcon) || 
                          (movingIconCount >= 10 && (score < 2000 || isMovingIconUpgraded))}
              ...
              >
                ...
                <span className="text-xs text-center">
                  {movingIconCount < 10 ? '移动图标' : (isMovingIconUpgraded ? '已升级' : '升级图标')}
                </span>
                <span className="text-xs text-gray-500">
                  {movingIconCount < 10 ? `${UNLOCK_THRESHOLDS.movingIcon}分` : 
                   (isMovingIconUpgraded ? '+5积分/次' : '2000分')}
                </span>
              </button>
            </div>
            ...
            <div className={`relative rounded-lg overflow-hidden w-24 h-24 border-2 ${
              ...
              <button 
                ...
                disabled={!availableComponents.dragonBall || 
                          score < (isDragonBallUpgraded ? 
                ...
                <span className="text-xs text-gray-500">
                  {dragonBalls.length < 7 ? 
                    `${isDragonBallUpgraded ? 
                      dragonBallCost * Math.pow(2, dragonBalls.length) : 
                      dragonBallCost + (dragonBalls.length * 500)}分` : 
                    '已满'}
                </span>
              </button>
            </div>
